...
\begin{enumerate}
	...
	\begin{itemize}
		\item The preprocesser recursively includes files marked by \texttt{\#include}
		\item The preprocesser expands macros such as \texttt{\#if}, \texttt{\#define}, \texttt{\#ifdef}
	\end{itemize}
...
\begin{itemize}
	\item \textbf{Garbage collection}: Detect data objects that cannot be used by the program in the future, and free its occupied resources.
	\item Always \texttt{free} allocated memory
	\begin{cpp}
...
free (p);
    ...
    It is good practice to never change the allocated pointer. Instead use a copy for doing stuff so you can always free the memory.
	\begin{cpp}
